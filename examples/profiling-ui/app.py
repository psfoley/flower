...
def _load_federation_config(
    ...
    config, _warnings = load_and_validate(pyproject_path, check_module=False)
    if (
        "tool" not in config
        or "flwr" not in config["tool"]
        or "federations" not in config["tool"]["flwr"]
    ):
        if address is None:
            raise ValueError(
                "No federation configuration found in pyproject.toml. "
                "Provide --address."
            )
        return SuperLinkConnection(
            name=federation or "manual",
            address=address,
    ...
    if "address" not in federation_config or federation_config["address"] is None:
        raise ValueError(
            "`profile-ui` requires a SuperLink address in federation configuration."
        )

    root_certificates = federation_config.get("root-certificates")
    if root_certificates is not None:
        root_certificates = str((app_path / root_certificates).resolve())

    return SuperLinkConnection(
        name=federation,
        address=federation_config["address"],
        root_certificates=root_certificates,
        insecure=bool(federation_config.get("insecure", False)),
        federation=federation_config.get("federation"),
    )
...
def create_app(
    ...
    superlink_connection = _load_federation_config(
