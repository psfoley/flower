...
def profile(
    ...
    try:
        ...
        config, warnings = load_and_validate(pyproject_path, check_module=False)
        for warning in warnings:
            typer.secho(f"Warning: {warning}", fg=typer.colors.YELLOW)
        federation, federation_config = validate_federation_in_project_config(
            federation, config, federation_config_overrides
        )
        address = federation_config.get("address")
        if address is None:
            raise ValueError(
                "`flwr profile` requires a SuperLink address in federation config."
            )
        root_certificates = federation_config.get("root-certificates")
        if root_certificates is not None:
            root_certificates = str((app / root_certificates).resolve())
