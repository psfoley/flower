    ...
    def aggregate_train(  # pylint: disable=too-many-locals
        ...
        for msg in valid_replies:
            ...
            if sum_delta is None:
                ...
                sum_delta = [sd + d for sd, d in zip(sum_delta, delta, strict=False)]
            sum_h += h
        ...
        array_list = [
            ...
            for gw, d in zip(global_weights, sum_delta, strict=False)
        ]
        ...
        return (
            ArrayRecord(OrderedDict(zip(array_keys, array_list, strict=False))),
            metrics,
        )
...
def compute_delta_and_h(
    ...
    for gw, lw in zip(global_weights, local_weights, strict=False):
