...
class DifferentialPrivacyAdaptiveBase(Strategy, ABC):
    ...
    def _add_noise_to_aggregated_arrays(self, aggregated: ArrayRecord) -> ArrayRecord:
        ...
        return ArrayRecord(
            OrderedDict(
                {k: Array(v) for k, v in zip(aggregated.keys(), nds, strict=False)}
            )
        )
    ...
    def aggregate_train(
        ...
        for reply in replies_list:
            for arr_name, record in reply.content.array_records.items():
                ...
                model_update = [
                    np.subtract(x, y)
                    for (x, y) in zip(reply_nd, current_nd, strict=False)
                ]
                ...
                restored = [
                    c + u for c, u in zip(current_nd, model_update, strict=False)
                ]
                reply.content[arr_name] = ArrayRecord(
                    OrderedDict(
                        {
                            k: Array(v)
                            for k, v in zip(record.keys(), restored, strict=False)
                        }
                    )
                )
            log(
