...
class FedAvgM(FedAvg):
    ...
    def aggregate_train(
        ...
        if self.server_opt and aggregated_arrays is not None:
            ...
            pseudo_gradient = [
                old - new
                for new, old in zip(aggregated_ndarrays, ndarrays, strict=False)
            ]
            if self.server_momentum > 0.0:
                if self.momentum_vector is None:
                    ...
                    self.momentum_vector = [
                        ...
                        for mv, pg in zip(
                            self.momentum_vector, pseudo_gradient, strict=False
                        )
                    ]
            ...
            updated_array_list = [
                ...
                for old, pg in zip(ndarrays, pseudo_gradient, strict=False)
            ]
            aggregated_arrays = ArrayRecord(
                OrderedDict(zip(array_keys, updated_array_list, strict=False))
            )
